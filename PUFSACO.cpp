...
void leer_preprocesado(vector<umap> &a_documentos, vector_string &a_clases) {
    ifstream infile("pre_eric_ds.csv");
    string line;
    string buffer;
    string word;
    double val;
    int iterator_by_comma;
    ...
    while (std::getline(infile, line))
    {
        ...
        while(getline(ss, buffer, ',')) {
            // primero cogemos la clase
            if(iterator_by_comma==0) {
                document_class = buffer;
                iterator_by_comma++;
                continue;
            }
            // cogemos la primera palabra
            if(iterator_by_comma==1) {
                word = buffer.substr(4, buffer.size()-5); //empezamos desde 4 x el corchete
                getline(ss, buffer, ',');
        ...
        cout << a_caracteristicas.size() << endl;

        a_documentos.push_back(a_caracteristicas);
        a_clases.push_back(document_class);
...
int main()
{
    ...
    int num_hormigas = 8;
    double tao = 0.2;
    int num_caracteristicas_final = 30;
    int num_caracteristicas_limite = 50;
    double q_ini = 0.7;
    double beta = 0.45;
    double evaporacion=0.2;

    omp_set_dynamic(0);     // Explicitly disable dynamic teams (no auto threads)
    omp_set_num_threads(16); 
    
    vector<umap> a_documentos;
    vector_string a_clases;
    leer_preprocesado(a_documentos, a_clases);
    ...
    ofstream outfile_matrix("matriz_"+str_time+".csv");
    ...
    for ( ; it != a_documentos.end(); ++it) {
        ...
        for (int nCiclo=0; nCiclo < num_ciclos; ++nCiclo){
            ...
            for(int i=0; i<num_hormigas; ++i) {
                for(int j=0; j<20-1; ++j) {
                    ...
                    // actualizamos array de visitados de la hormiga y la posición  de la caracteristica escogida
        ...
        i = 0;
        outfile << a_clases[num_cont_documentos] << ",";
    ...
    for (auto word : bag_of_words)
        outfile_matrix << word.first << ",";
    outfile_matrix << "_clase_del_doc";
    outfile_matrix << endl;

    cout <<"num_cont_documentos: " << num_cont_documentos << endl;
    for (int i=0; i< num_cont_documentos; ++i) {
        bool documento_vacio = true;
        for (auto word : bag_of_words) {
            if (word.second.count(i)) {
